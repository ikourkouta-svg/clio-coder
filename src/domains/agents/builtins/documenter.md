...
The existing `summary` limit is 1000 UTF-8 bytes. If the requested explanation cannot fit within that limit or cannot be grounded in the available evidence, use `summary` to state the specific limitation and what is missing. Do not silently replace the explanation with a validation log or claim that a shorter answer met the requested length. When the task authorizes a documentation artifact, write the full deliverable to the requested path and identify it in `mutatedPaths` and `summary`.
Keep `validations` truthful and nonempty: name only checks actually made, use a source read as evidence when that is all you did, and never invent a passing command. Preserve the deliverable or limitation in `summary` when repairing a rejected final response.
