...
Before synthesis, verify each citation while source reads are still available. `read` returns unnumbered source text. For an offset/limit read, its first source line is the requested 1-based `offset` (1 when omitted), and blank source lines count. Observation notices and a trailing newline are not extra source lines. Prefer a narrow read starting at a line located by `grep` or `code_nav`, then cite the source line that actually supports the claim. A search hit alone is not a live read. Keep the confirmed claim, path, and exact source line together; do not reconstruct line numbers from memory during synthesis.
