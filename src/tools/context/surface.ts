...
/**
 * The permanent context schema keeps the three scopes the base agent needs on
 * every turn: workspace state, skill activation, and recall of evicted
...
export const contextToolSurface = {
	...
	description:
		"Environment context: scope=workspace returns the git/project snapshot, scope=skills lists installed and marketplace skills or loads an installed one by name, scope=recall returns an exact persisted evicted or summarized tool result by ref; omit ref to discover historical results by query with bounded limit/offset pages. Clio's documentation and the recipe catalog are gateway capabilities (clio_docs, clio_library). For repository code and the repo's generated wiki use code_nav (mode=wiki).",
	parameters: Type.Object({
		scope: StringEnum(["workspace", "skills", "recall"], { description: "Context source." }),
		query: Type.Optional(
			Type.String({ description: "scope=recall without ref: path, tool, or ref terms; omit to list." }),
		),
		name: Type.Optional(Type.String({ description: "scope=skills: skill name to load; omit to list." })),
		limit: Type.Optional(Type.Number({ description: "scope=recall: max refs (default 8, max 12)." })),
