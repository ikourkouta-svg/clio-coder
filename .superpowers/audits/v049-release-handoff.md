...
## Recorded proof session, 2026-09-17

One real TUI session ran on the build of `d1e92518` under `asciinema session`, project root `/var/tmp/c49proof`, orchestrator `mini/qwopus3.8-27b-dense-q6` through `blade-gateway`. Four parallel scout workers on `dynamo/qwen3.8-27b` were placed on the SSH node `dragon` by the least-loaded rule; three live `ssh -T dragon` worker channels were observed at once and no remote process was stranded. CiteNexus 0.2.2 (local worktree) served citations over MCP: two calls with an undeclared argument were rejected by schema, the model recovered through `gateway(op="describe")`, and results entered context at 2.2 to 2.9 KB. WTF-P 0.7.3 (local branch) produced a laid-out HTML poster with a source-attributed diagram, per-claim file and line references, visible placeholders for the missing affiliation, contact and funding, and no uncited comparison. The turn finished in 871 s at 76% context. The session ledger stores the typed `/wtfp:create-poster ...` input as `displayText`. Evidence is under `proof-d1e92518/` in the external archive: `proof.cast`, `poster.png` and the deliverable.
...
3. The read-only exploration nudge counts a Scout dispatch as successful only when the whole dispatch result is `ok`. A batch with one failed member out of four still produced the advisory "without a successful Scout dispatch" in the proof session. It is an info reminder, not a gate.
4. One scout in the proof session failed its result contract with invalid JSON, a model output fault the orchestrator recovered from by reading the files itself. SSH multi-node parallel placement is now exercised live; SSH cancellation and failover still are not.
