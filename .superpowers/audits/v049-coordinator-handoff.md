...
Next: run the release gates, repair the recorded hygiene failures without weakening checks, and only after green CI cut version 0.4.9 following `docs/process/release-cut-checklist.md`. That process requires a clean local candidate commit, frozen-lockfile install, and `pnpm run ci:release`. No remote push or publication has been authorized. The package version remains 0.4.8 until the cut. `v049-hygiene` in pane `wR:p2E` is idle after prompt repair and ready for `brief-hygiene.md`; `v049-docs` in `wR:p2J` is idle after accepted documentation corrections.

Release-gate evidence: the first fast lane passed typecheck and failed lint with four drift conditions, recorded in scratchpad `gate-fast-initial-2.log` and `gate-fast-initial.json`. Build and fast tests were skipped in that failed attempt. The ten-file hygiene repair subsequently passed independent astra review, all 16 lint/hygiene checks, 223 focused tests, and the prompt-guard rejection fixtures. The repair corrects the publication namespace, removes unused exports/functions, and updates the stale docs-routing guard without weakening it. Biome retains the existing two warnings and two infos. The complete fast lane, full suite, and CI are next; version 0.4.8 remains unchanged.
