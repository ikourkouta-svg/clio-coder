...
| T | post-closeout workbench ergonomics | Codex tui-workbench | editor, welcome/footer, worker/retry renderers and labels per scratch brief | actionable activity and identity, useful editor affordances, width-safe layout | COMMITTED in this workbench slice after T2 PASS; 147 independent tests; final CI and full suite PASS (2525 pass, 1 Windows-only skip); final live 43-column identity check PASS. |

| U | TUI design and guide integration | Codex tui-workbench | CHANGELOG.md, docs/architecture/tui-design.md, docs/guide/commands-and-modes.md | final R/S/T behavior documented without contradictory old contracts | REVIEW PASS; final R2/S2/T2 docs independently verified; integration CI/full suite PASS, coordinator audit record pending before commit. |
