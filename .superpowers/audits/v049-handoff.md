...
| R | post-closeout View ergonomics | Codex tui-view | `src/interactive/view/**` and owned browser tests | truthful filters, useful output identity, clear focus and narrow navigation | COMMITTED in this View slice after R2 PASS; 46 independent tests, frozen install, integrated CI (649 root pass, 1 skip; 24 web pass), and live filter/provenance/reset/undo PASS. |
| S | post-closeout Library ergonomics | Codex tui-library | `src/interactive/overlays/library*.ts` and owned Library tests | honest resource counts, browse/detail flow, Agents/Fleets clarity and safe actions | REFINING; Astra PASS WITH NITS, owner closing state-color/search-hint/count-unit polish findings before landing. |
| T | post-closeout workbench ergonomics | Codex tui-workbench | editor, welcome/footer, worker/retry renderers and labels per scratch brief | actionable activity and identity, useful editor affordances, width-safe layout | REPAIRING; Astra FAIL found production model-label collisions and control-sequence splitting; owner closing both with pipeline regressions. |

| U | TUI design and guide integration | Codex tui-workbench | CHANGELOG.md, docs/architecture/tui-design.md, docs/guide/commands-and-modes.md | final R/S/T behavior documented without contradictory old contracts | IMPLEMENTING; source slices independently reviewed before landing; documentation review pending. |
