...
| R | post-closeout View ergonomics | Codex tui-view | `src/interactive/view/**` and owned browser tests | truthful filters, useful output identity, clear focus and narrow navigation | COMMITTED 236e92a5 after R2 PASS; 46 independent tests, frozen install, integrated CI (649 root pass, 1 skip; 24 web pass), and live filter/provenance/reset/undo PASS. |
| S | post-closeout Library ergonomics | Codex tui-library | `src/interactive/overlays/library*.ts` and owned Library tests | honest resource counts, browse/detail flow, Agents/Fleets clarity and safe actions | COMMITTED c175855b after S2 PASS; 83 Library and 16 keyboard tests PASS; final CI 663 root pass/1 skip and 24 web pass; live narrow/wide browsing and fleet previews exercised. |
| T | post-closeout workbench ergonomics | Codex tui-workbench | editor, welcome/footer, worker/retry renderers and labels per scratch brief | actionable activity and identity, useful editor affordances, width-safe layout | COMMITTED 46a113d9 after T2 PASS; 147 independent tests; final CI and full suite PASS (2525 pass, 1 Windows-only skip); final live 43-column identity check PASS. |

| U | TUI design and guide integration | Codex tui-workbench | CHANGELOG.md, docs/architecture/tui-design.md, docs/guide/commands-and-modes.md | final R/S/T behavior documented without contradictory old contracts | COMMITTED in this documentation slice after U PASS; final CI/full suite PASS; live outcomes and failures recorded in v049-release-gates.md; exact qualification governed by the matching external receipt. |

| V | Maintenance: late addressed worker steering | Codex maint-steer | `src/interactive/editor-steer.ts`, `dispatch-steering.ts`, `editor-submit.ts`, focused routing tests | terminal or stale worker instructions never become an unintended main turn; active steering and file references preserved | COMMITTED cb8e08bd after independent Astra XHigh PASS; 20 owned tests, six extra boot/busy cases and actual TUI rejection/unchanged ledger; integrated CI PASS. |
| W | Maintenance: concise provider diagnostics | Codex maint-errors | retry renderer, chat panel, focused error projection/tests | actionable bounded presentation with full diagnostic evidence and truthful retry layer | COMMITTED 0062e595 after independent Astra XHigh Standards/Specification PASS; all four prior blockers and actual replay outcome defect closed; 72 combined presentation tests, nine unchanged independent/SDK tests and actual ledger check pass. |

| X | Maintenance: session persistence integrity | Codex maint-session | `src/engine/session.ts`, session admission and focused contracts | short writes cannot report durable success; corrupt resume preserves current session | COMMITTED 230cc1c8 after X2 independent Standards/Specification PASS; all four prior blockers closed; 69 review tests plus payload/normalization probes pass. Existing two-step metadata publication limit recorded separately. |
| Y | Maintenance: provider probe body deadlines | coordinator investigation | provider HTTP probe and focused tests, owner pending | timeout and caller cancellation cover body consumption; successful HTTP probes release body ownership | COMMITTED 8e2f6b38 after independent Astra XHigh PASS; 20 local HTTP tests, redirects/204/cleanup and 180-request status investigation; integrated CI PASS. |

| Z | Maintenance: session picker identity | Codex maint-steer | session-selector and focused picker tests | task identity survives narrow layouts, filtering preserves selection, hostile metadata safe | COMMITTED e96e1c22 after independent Standards/Specification PASS; 19 actual-overlay tests and rebuilt 43-column picker dogfood pass. |
| AA | Maintenance: completed worker previews | Codex maint-errors, queued | dispatch-board, shared worker presentation and focused tests | readable recognized contracts without loss of raw evidence or validation honesty | COMMITTED abfba708 after principal closure of three independent P2 findings; 70 combined closure cases and complete full suite PASS. Other Astras stopped by explicit operator request; no fresh headless closure verdict claimed. |
| AB | Maintenance: skill activation prompt consistency | Codex maint-session, queued | prompt compiler, skills fragment and focused tests | compiled guidance agrees with existing autonomy permission; marketplace stays operator-gated | COMMITTED badbbceb after independent ACAB Standards/Specification PASS (29 reviewer tests); principal full-suite assertion synchronized with reviewed policy. Mandatory first-turn discovery preserved. |
| AC | Maintenance: native probe deadline gaps | owner pending | llama.cpp embed/rerank, reasoning body cleanup and focused tests | direct requests retain timeout, cancellation and body ownership | COMMITTED 15966437 after independent ACAB Standards/Specification PASS (67 reviewer tests); per-request deadlines, cleanup and no duplicate inference controls pass. |

| AD | Maintenance: scheduler launch ownership | Codex maint-session | execution-scheduler and focused tests | cancellation/failure owns started and late handles through terminal cleanup, no premature release | COMMITTED d206d13a with AE after principal closure of independent P1 retry-ownership finding at real fleet adapter; queued/admitting/running continuation tests and full suite PASS. Optional typed member drain seam approved. |
| AE | Maintenance: fleet member control identity | Codex maint-session | dispatch extension and focused tests | concrete member steering/abort/permission cannot target a sibling under shared fleet lineage | COMMITTED d206d13a with AD after principal closure of independent P1 nonzero repair-ordinal control collision; genuine assignment retry controls retained; full suite PASS. |
