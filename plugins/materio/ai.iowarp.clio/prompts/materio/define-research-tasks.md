...
if [ ! -f .research/VIRTUAL-LAB.md ]; then
  echo "NOTE: No VIRTUAL-LAB.md found. Run /materio:define-virtual-lab to map your resources first."
...
Writing tasks need no interview; they bridge to `/materio:wtfp`.
...
## 6. Spawn materio-workflow-planner Agent
...
Use dispatch with `agent: "materio-workflow-planner"` and a task containing the assignment and full context. The prompt carries:
...
- `/materio:remove-task [N]`; remove a task
- `/materio:archive-task [N]`; archive without deleting
