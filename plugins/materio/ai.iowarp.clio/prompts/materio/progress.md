...
```bash
[ ! -f .research/RESEARCH.md ] && echo "No research project. Run /materio:identify-research." && exit 0
...
**Phase = exploring:** → `/materio:literature-review`
**Phase = reviewing:** → `/materio:define-virtual-lab` if VIRTUAL-LAB.md is missing, otherwise `/materio:define-research-tasks`
**Phase = executing, task in-progress:** → `/materio:execute-task [N]`
**Phase = executing, next pending task:** → `/materio:execute-task [N+1]`
**Phase = writing:** → `/materio:wtfp` or `/wtfp:progress`
