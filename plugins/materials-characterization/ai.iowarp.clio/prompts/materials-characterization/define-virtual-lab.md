...
**Orchestrator role:** Load research context, run the resource interview (quick dump or guided by category, with questions targeted to the methods the field uses), spawn the materials-characterization-lab-definer agent to structure and gap-analyze, present the summary and gaps for verification, re-dispatch the agent with corrections, record.
...
## 4. Spawn materials-characterization-lab-definer Agent
...
Use dispatch with `agent: "materials-characterization-lab-definer"` and a task containing the assignment and full context. The prompt carries:
...
`/materials-characterization:define-research-tasks`
...
**Also available:**
- `/materials-characterization:upload-data`; register existing datasets before planning tasks
- `/materials-characterization:literature-review`; continue reviewing if not complete
