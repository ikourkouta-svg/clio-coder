---
version: 4
name: materials-characterization-execute-task
description: Execute one pre-approved research task and independently inspect its output files.
steps:
  - kind: agent
    id: execute
    agent: materials-characterization-task-executor
    scope: workspace
    writes: [.research/tasks/]
    dependencies: []
  - kind: agent
    id: verify
    agent: materials-characterization-task-verifier
    scope: readonly
...
remove the writes boundary. Use `/materials-characterization:execute-task` instead:
...
Use `/materials-characterization:execute-task` for the complete interactive workflow.
