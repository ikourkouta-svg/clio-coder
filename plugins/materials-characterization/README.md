...
Turn a materials research question into a supplied-corpus literature synthesis,
...
Install the whole package with `clio-coder plugins install ./plugins/materials-characterization --project`
...
1. `/materials-characterization:identify-research` interviews domain, hypothesis,
   scope, resources, and timeline; the researcher selects a calibrated question.
2. `/materials-characterization:upload-data` indexes supplied papers/data and
   optionally copies files without overwriting collisions.
3. `/materials-characterization:literature-review` reviews supplied materials and
   authorized URLs, preserving actual inspection depth and justified gaps.
4. `/materials-characterization:define-virtual-lab` maps equipment, compute,
   licenses, collaborators, consumables, people, access constraints, and alternatives.
5. `/materials-characterization:define-research-tasks` customizes a traditional
   workflow, interviews per-task assumptions, and confirms defaults/resource gaps.
6. `/materials-characterization:execute-task N` prepares the approved task's
   output contract, reads actual artifacts, and presents advisory findings.
7. `/materials-characterization:wtfp` prepares author-reviewed new-paper,
...
The optional `materials-characterization-execute-task` fleet takes `task`,
...
materials-characterization-research-explorer skill.” It does not register Codex
...
`materials-characterization:research-explorer`. Paper briefs name receiving wtf-p
...
```bash
PYTHONDONTWRITEBYTECODE=1 python3 -B -m unittest discover -s plugins/materials-characterization/tests -v
