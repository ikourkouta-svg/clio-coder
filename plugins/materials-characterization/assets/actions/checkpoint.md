...
`python3 "${PACKAGE_ROOT}/assets/scripts/research_state.py" checkpoint save LABEL`.
...
For list, run `python3 "${PACKAGE_ROOT}/assets/scripts/research_state.py" checkpoint list`
...
Collect confirmation with the host conversation facility. Cancellation leaves state untouched. Save a
`before-restore` checkpoint, then run
`python3 "${PACKAGE_ROOT}/assets/scripts/research_state.py" checkpoint restore NAME --confirmed`.
