...
**Orchestrator role:** Load research context, present workflow options, gather customizations and scope, run the per-task assumption interview, apply the materials-characterization-workflow-planner skill to generate WORKFLOW.md, present resource flags and planner defaults for decision, apply the role again with revisions, create task directories, record.
...
if [ ! -f .research/VIRTUAL-LAB.md ]; then
  echo "NOTE: No VIRTUAL-LAB.md found. Run Run define-virtual-lab using the materials-characterization-lab-definer skill to map your resources first."
...
Writing tasks need no interview; they bridge to `Run wtfp using the materials-characterization-research-explorer skill`.
...
## 6. apply the materials-characterization-workflow-planner skill
...
Apply the materials-characterization-workflow-planner role skill using the assignment and full context; delegate if supported. The prompt carries:
...
`Run execute-task using the materials-characterization-task-executor skill 1`
...
- `Run remove-task using the materials-characterization-research-explorer skill [N]`; remove a task
- `Run archive-task using the materials-characterization-research-explorer skill [N]`; archive without deleting
