...
**Orchestrator role:** Resolve target task, check dependencies, gate-confirm assumptions, apply the materials-characterization-task-executor skill with full task context, verify outputs exist, run guardrails and surface findings, update WORKFLOW.md and STATE.md, checkpoint, record.
...
```bash
[ ! -f .research/WORKFLOW.md ] && echo "ERROR: No WORKFLOW.md. Run Run define-research-tasks using the materials-characterization-workflow-planner skill first." && exit 1
...
- options: "Proceed" | "Update assumptions first" | "I need to upload data first; Run upload-data using the materials-characterization-research-explorer skill" | "Skip this task"
...
**If type = `writing`:**
- Tell user: "This task involves writing. Use `Run wtfp using the materials-characterization-research-explorer skill` to bridge into wtf-p for paper writing."
- Exit gracefully.
...
**If type = `data-analysis`:** check for registered data in DATA-INDEX.md and `.research/data/`; if none, stop and offer `Run upload-data using the materials-characterization-research-explorer skill` → analysis code + results summary + figure descriptions

**If type = `analytical`:** theoretical mode → derivation, validation, optional implementation

## 6. apply the materials-characterization-task-executor skill
...
Apply the materials-characterization-task-executor role skill using the assignment and full context; delegate if supported. The prompt carries:
...
1b. Apply the materials-characterization-task-verifier skill to the approved task,
...
`Run execute-task using the materials-characterization-task-executor skill `

**Or check progress**
`Run progress using the materials-characterization-research-explorer skill`
