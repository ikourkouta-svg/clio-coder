            ...
            for target in ["codex", "claude", "gemini"]:
                ...
                for action, invocation in report["invocations"].items():
                    if target == "codex":
                        ...
                        self.assertEqual(invocation["name"], f"/materials-characterization:{action}")
                        path = output / "commands" / (action + ".md") if target == "claude" else output / "commands/materials-characterization" / (action + ".toml")
                        self.assertTrue(path.is_file())
                if target == "codex":
                    self.assertNotRegex(guide_text, r"/(?:materials-characterization|wtfp):[a-z-]+")
                    self.assertIn("wtf-p action new-paper", guide_text)
                if target == "claude":
                    registered = {"materials-characterization:" + path.stem for path in (output / "agents").glob("*.md")}
                    references = set(re.findall(r"registered agent `([^`]+)`", guide_text))
                    self.assertTrue(references)
                    self.assertTrue(references.issubset(registered))
                    self.assertNotRegex(guide_text, r"(?i)\b(?:spawn|dispatch) (?:the |a fresh )?materials-characterization-[a-z-]+")
                else:
    ...
    def test_all_peer_packages_have_resolvable_skill_assets_and_truthful_reports(self):
        with tempfile.TemporaryDirectory(prefix="materials peers ") as temp:
            for target in ["codex", "claude", "gemini"]:
                ...
                if target == "codex":
                    self.assertEqual(json.loads((output / "plugin.json").read_text())["name"], "materials-characterization")
                if target == "claude":
