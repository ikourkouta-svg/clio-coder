...
## Use this library from Claude Code
...
## Import an explicit foreign package

Adoption above starts from what an installed host already has. When you have a
specific package instead, a local directory or a GitHub tree URL, import it
directly:
...
In a session, `/library import <path-or-url>` reviews that exact source in the
Library. The review names the projected resources, every unsupported host
feature and every omitted file before anything is written; cancelling releases
the staged copy of the vendor tree and writes nothing. The Library's `o` key is
the separate route into the local-agent discovery surface below, which starts
from what an installed host already has rather than from a source you name.
