...
Architecture mapping through Archify follows an explicit honesty boundary across four distinct stages: deterministic structural seed generation (`clio-coder context map`), model-authored refinement, deliverable validation (`validate`, `deliver`, and Clio verifiers), and human visual review. Clio derives the deterministic architecture seed from the structural codewiki without model calls; Archify validates and renders the model's semantic refinements. Standard validation checks schema conformance (such as the 9-check mechanical pass with 0 errors and 45 non-fatal warnings), but schema validation does not prove visual readability or layout correctness. Custom model-authored maps can remain layout-invalid or visually broken until iteratively refined; visual and semantic acceptance requires browser inspection and operator review rather than relying on mechanical exit codes or sealed artifacts alone.
