...
test("docs: containment, static methods and types, menu synthesis and topic resolution", async () => {
	...
	await writeFile(
		join(html, "alpha_blueprint.html"),
		'<meta content="docs/guide/a.md" name="clio-markdown-source"><h1>Alpha</h1>\n',
	);
	...
	try {
		...
		assert.equal(page.links["../html/alpha_blueprint.html"], "/docs/blueprints/alpha_blueprint.html");
		...
		assert.equal((await raw("/docs-html/")).headers.location, "/docs");
		const get = await raw("/docs-html/index.html?embed=1");
		assert.equal(get.status, 200);
		assert.ok(get.body.includes("<h1>Clio docs</h1>"));
		assert.ok(get.body.includes("clio:blueprint"));
		...
		const head = await raw("/docs-html/index.html?embed=1", "HEAD");
		...
		assert.equal(menu.items[0]?.documentPath, "guide/a.md");
		assert.equal(menu.items[1]?.documentPath, undefined);
		for (const path of ["alpha", "ALPHA.html", "beta", "Beta.HTML"]) {
			const redirect = await raw(`/docs-html/${path}`);
			assert.equal(redirect.status, 302, path);
			assert.match(redirect.headers.location ?? "", /^\/docs\/blueprints\/(?:alpha_blueprint\.html|Beta\.HTML)$/);
			assert.equal((await raw(`/docs-html/${path}?embed=1`)).status, 200, path);
		}
...
test("docs: incomplete installations report absent blueprints and docs explicitly", async () => {
	const fixture = await scratchHome();
	...
	try {
		...
		for (const [source, { page }] of pages) {
			...
			for (const destination of Object.values(page.links)) {
				...
				if (url.pathname.startsWith("/docs/blueprints/"))
					blueprints.add(url.pathname.replace("/docs/blueprints/", "/docs-html/"));
				else if (url.pathname === "/docs") assert.ok(pages.has("README.md"));
		...
		for (const path of blueprints) {
			assert.equal((await h.request(path)).status, 302, path);
			const response = await h.request(`${path}?embed=1`);
			assert.equal(response.status, 200, path);
			const original = await readFile(join(root, "html", decodeURIComponent(path.slice("/docs-html/".length))), "utf8");
			const content = await response.text();
			assert.ok(
				content.includes(original.slice(original.indexOf("<body"), original.lastIndexOf("</body>"))),
				`${path} preserves the authored body`,
			);
		}
		const menu = await json(await h.request("/api/docs/blueprints"), Blueprints);
		assert.equal(menu.items.length, blueprints.size);
		for (const row of menu.items)
			assert.ok(row.documentPath && pages.has(row.documentPath), `${row.file} has a canonical paired document`);
