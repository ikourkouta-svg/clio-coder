...
const source = fileURLToPath(new URL("../../plugins/materials-characterization/", import.meta.url));
...
describe("materials characterization plugin", () => {
	afterEach(() => {
	...
	it("loads the complete installed graph through native production resource loaders", () => {
		...
		const result = installPlugin(source, {
			...
			expectedId: "materials-characterization",
		});
		...
		for (const prompt of prompts.items) {
			ok(prompt.name.startsWith("materials-characterization:"));
		...
		const verifier = recipes.find((recipe) => recipe.id === "materials-characterization-task-verifier");
		strictEqual(verifier?.capabilityClass, "read-only");
		ok(verifier?.tools.every((tool) => !["write", "edit", "bash", "ask_user"].includes(tool)));
		const fleetPath = path.join(root, "ai.iowarp.clio/fleets/materials-characterization-execute-task.md");
