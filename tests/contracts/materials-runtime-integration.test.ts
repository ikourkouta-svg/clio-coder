...
it("reloads the actual materials bundle through the interactive slash runtime and refreshes native bound roles", async () => {
	const env = await isolateClioEnv("clio-materials-runtime-");
	...
	try {
		...
		const prompts = loadPromptTemplates({ cwd, home: env.dir }).items.filter((x) =>
			x.name.startsWith("materials-characterization:"),
		);
		...
		disablePlugin("materials-characterization", { cwd, scope: "project" });
