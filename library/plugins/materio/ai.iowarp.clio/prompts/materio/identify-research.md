...
**Orchestrator role:** Check preconditions, detect existing research context, run the whole researcher interview (foundation, profile, and Socratic drilling), spawn the materio-research-explorer agent for synthesis, present its candidate prompts, collect the choice, re-dispatch the agent to write the files, verify, record.
...
```bash
[ -f .research/RESEARCH.md ] && echo "WARN: Research already initialized. Use /materio:progress to see current state, or continue to refine."
...
If data files found: ask via ask_user whether to base the research framing on existing files, and suggest `/materio:upload-data` afterwards so the literature review can read them.
...
## 6. Dispatch materio-research-explorer
...
Use dispatch with `agent: "materio-research-explorer"` and a task containing the assignment and full context. The prompt carries the full inlined context:
...
Re-dispatch the explorer with a `<selection>` block containing the choice and any corrections. Always spawn a fresh materio-research-explorer with the full `<researcher_input>` transcript plus `<selection>`.
...
`/materio:upload-data` (optional, but the review is only as good as its corpus)
`/materio:literature-review`
...
**Also available:**
- `/materio:define-research-tasks`; skip literature review if already done
